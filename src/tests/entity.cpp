...
PR_TEST("Parent");
{
	...
	PR_CHECK_NEARLY_EQ_3(child->position(), PM::pm_Set(0, 3, 0, 1));
	...
	PR_CHECK_NEARLY_EQ_3(child->position(), PM::pm_Set(0, 5, 0, 1));

	root->setRotation(PM::pm_RotationQuatRollPitchYaw(PM::pm_Set(0, 0, PM::pm_DegToRad(90))));
	PR_CHECK_NEARLY_EQ_3(child->position(), PM::pm_Set(-4, 1, 0, 1));
