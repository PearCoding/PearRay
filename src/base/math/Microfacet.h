...
inline float g_1_smith_opt(float NdotK, float roughness)
{
	...
	const float denom = NdotK + std::sqrt(a + b - a * b);
	return (denom <= PR_EPSILON) ? 0.0f : 1.0f / denom;
}
...
inline Vector3f sample_ndf_ggx(float u0, float u1, float roughness)
{
	float cosTheta, sinTheta; // V samples
	float cosPhi, sinPhi;	  // U samples
...
inline Vector3f sample_ndf_ggx(float u0, float u1, float roughnessX, float roughnessY)
{
	float cosTheta, sinTheta; // V samples
	float cosPhi, sinPhi;	  // U samples

	const float phi = std::atan(roughnessY / roughnessX * std::tan(PR_PI + 2 * PR_PI * u0)) + PR_PI * std::floor(2 * u0 + 0.5f);
	sinPhi			= std::sin(phi);
	cosPhi			= std::cos(phi);

	const float f1	   = cosPhi / roughnessX;
	const float f2	   = sinPhi / roughnessY;
	const float alpha2 = 1 / (f1 * f1 + f2 * f2);
	const float t2	   = alpha2 * u1 / (1 - u1);
	cosTheta		   = std::max(0.001f, 1.0f / std::sqrt(1 + t2));
	sinTheta		   = std::sqrt(1 - cosTheta * cosTheta);
								...
								float roughnessX, float roughnessY)
{
	...
	return Vector3f(roughnessX * Nh(0), roughnessY * Nh(1), std::max(0.0f, Nh(2))).normalized();
}
