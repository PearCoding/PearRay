...
public:
	...
	inline Vector2f mapGlobalToLocalUV(const Vector2f& uv) const
	{
		...
		const float det = diffProd(duv2(1), duv1(0), duv1(1), duv2(0));
		if (std::abs(det) < PR_EPSILON)
			return UV[0];

		float nu = diffProd(duv2(1), uv(0) - UV[0](0), duv2(0), uv(1) - UV[0](1));
		float nv = diffProd(duv1(0), uv(1) - UV[0](1), duv1(1), uv(0) - UV[0](0));
