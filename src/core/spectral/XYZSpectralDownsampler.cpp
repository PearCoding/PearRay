...
struct _SpectralMapperInternal {
	uint32 Resolution;
	float* Scale;
	float* Data;
};

SpectralMapper::SpectralMapper(const std::wstring& filename)
	: mInternal(new _SpectralMapperInternal())
{
...
void SpectralMapper::prepare(const float* r, const float* g, const float* b, float* out_a, float* out_b, float* out_c, size_t elems)
{
...
void SpectralMapper::compute(const float* a, const float* b, const float* c, const float* wavelengths, float* out_weights, size_t elems)
{
#pragma GCC ivdep
	for (size_t i = 0; i < elems; ++i) {
...
void SpectralMapper::toXYZ(const float* weights, const float* wavelengths, float* x, float* y, float* z, size_t elems)
{
#pragma GCC ivdep
	for (size_t i = 0; i < elems; ++i) {
		float t = (wavelengths[i] - WAVELENGTH_START) / WAVELENGTH_DELTA;
		float tf;
		uint32 ti = std::modf(t, &tf);

		float fx = NM_TO_X[ti];
		float fy = NM_TO_Y[ti];
		float fz = NM_TO_Z[ti];
		if (ti < SAMPLE_COUNT - 1) {
			fx = tf * fx + (1 - tf) * NM_TO_X[ti + 1];
			fy = tf * fy + (1 - tf) * NM_TO_Y[ti + 1];
			fz = tf * fz + (1 - tf) * NM_TO_Z[ti + 1];
		}
...
void SpectralMapper::toXYZComposite(const float* weights, const float* wavelengths, float& x, float& y, float& z, size_t elems)
{
	x = 0;
	y = 0;
	z = 0;
