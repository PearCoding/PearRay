...
void PropertyView::setPropertyTable(PropertyTable* table)
{
	...
	if (mProperties)
	{
		connect(mProperties, SIGNAL(propertyStructureChanged(IProperty*)),
			this, SLOT(propertyStructureWasChanged(IProperty*)));
		connect(mProperties, SIGNAL(propertyChanged(IProperty*)),
			this, SLOT(propertyStructureWasChanged(IProperty*)));
...
QTreeWidgetItem* PropertyView::setupItem(QTreeWidgetItem* item, IProperty* property)
{
	...
	else if (property->hasNoName())
	{
		item->setFirstColumnSpanned(true);
		QWidget* editor = property->editorWidget(this);
		if (editor)
		{
			setItemWidget(item, 0, editor);
		}
		else
		{
			item->setText(0, property->valueText());
		}
	}
	else
	{
...
void PropertyView::propertyStructureWasChanged(IProperty* obj)
{
	QTreeWidgetItem* item = mMapper.key(obj, nullptr);
