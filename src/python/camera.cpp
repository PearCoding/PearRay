...
namespace PRPY
{
    ...
    void setup_camera()
    {
        bpy::class_<CameraWrap, bpy::bases<Entity>, boost::noncopyable>("Camera", bpy::init<uint32, std::string>())
            .def("constructRay", bpy::pure_virtual(&Camera::constructRay))
        ;

        bpy::class_<StandardCamera, bpy::bases<Camera>, boost::noncopyable>("StandardCamera", bpy::init<uint32, std::string>())
