...
namespace PR
{
	...
	RGBConverter::RGBConverter(GPU* gpu, size_t size, bool byte) :
	...
	void RGBConverter::convert(float* input, void* output, bool linear)
	{
#ifndef PR_NO_GPU
		if (mSize > 100)
		{
			try
			{
				cl::Kernel kernel(mProgram, linear ?
					(!mByte ? "m_srgb" : "m_srgb_byte") : 
					(!mByte ? "m_srgb_gamma" : "m_srgb_gamma_byte"));
		...
		else
		{
			...
			else
			{
				if (mByte)
				{
					...
					{
						float r, g, b;
						convertGAMMA(Spectrum(&input[i*Spectrum::SAMPLING_COUNT]), r, g, b);
				...
				else
				{
					for (size_t i = 0; i < mSize; ++i)
					{
						convertGAMMA(Spectrum(&input[i*Spectrum::SAMPLING_COUNT]),
							reinterpret_cast<float*>(output)[i * 3],
							reinterpret_cast<float*>(output)[i * 3 + 1],
							reinterpret_cast<float*>(output)[i * 3 + 2]);
					}
