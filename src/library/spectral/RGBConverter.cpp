...
namespace PR
{
	...
	void RGBConverter::convert(const Spectrum& s, float &x, float &y, float &z)
	{
		...
		/*x = 0.4124*X + 0.3576*Y + 0.1805*Z;
		y = 0.2126*X + 0.7151*Y + 0.0721*Z;
		z = 0.0193*X + 0.1192*Y + 0.9505*Z;*/

		// Gamma; No gamma here. Later maybe
		/*x = (x <= 0.0031308f) ? 12.92f*x : (1.055*pow(x, 1 / 2.4f) - 0.55f);
		y = (y <= 0.0031308f) ? 12.92f*y : (1.055*pow(y, 1 / 2.4f) - 0.55f);
		z = (z <= 0.0031308f) ? 12.92f*z : (1.055*pow(z, 1 / 2.4f) - 0.55f);*/
	}
