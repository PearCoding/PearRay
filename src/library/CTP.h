...
class t_sqrt {
	...
	{
		return (low == high ? low : ((v / (low + high + 1) < (low + high + 1) / 4) ? _helper(v, low, (low + high + 1) / 2 - 1) : _helper(v, (low + high + 1) / 2, high)));
	}
...
private:
	...
	{
		return ((i == 0) ? 0 : calc_length(i - 1) + ((number % i == 0) ? ((number / i == i) ? 1 : 2) : 0));
	}
	...
	struct Impl {
		typedef typename std::conditional<(number % M) == 0,
										  typename std::conditional<M == (number / M),
																	Impl<M - 1, M, Rest...>,
																	Impl<M - 1, M, Rest..., number / M>>::type,
										  Impl<M - 1, Rest...>>::type T;
