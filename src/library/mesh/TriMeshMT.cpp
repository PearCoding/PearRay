...
#include "geometry/TriangleIntersection_MT.h"
#include "mesh/MeshBase.h"

namespace PR {
TriMeshMT::TriMeshMT(const std::string& name,
...
void TriMeshMT::checkCollisionLocal(const RayPackage& in, CollisionOutput& out)
{
	...
	mCollider
		->checkCollisionIncoherent(in, out,
								   [&](const RayPackage& in2, uint64 f, CollisionOutput& out2) {
									   ...
									   bfloat hits		  = TriangleIntersection::intersectMT(in2, p0, p1, p2, uv, t);
...
void TriMeshMT::checkCollisionLocal(const Ray& in, SingleCollisionOutput& out)
{
	PR_ASSERT(mCollider && mBase, "Load before collision checks");
	mCollider
		->checkCollisionSingle(in, out,
							   [&](const Ray& in2, uint64 f, SingleCollisionOutput& out2) {
								   ...
								   bool hit = TriangleIntersection::intersectMT(in2, p0, p1, p2, uv, t);
