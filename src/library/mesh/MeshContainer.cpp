...
void MeshContainer::setFaceVertexCount(const std::vector<uint8>& faceVertexCount)
{
	...
	mFaceOffset.resize(faceVertexCount.size());
	mTriangleCount = 0;
	mQuadCount	 = 0;

	uint32 counter = 0;
	for (size_t i = 0; i < faceVertexCount.size(); ++i) {
		mFaceOffset[i]   = counter;
		uint32 faceElems = faceVertexCount.at(i);
...
float MeshContainer::faceArea(size_t f, const Eigen::Affine3f& tm) const
{
	...
	uint32 faceElems = faceVertexCount(f);

	uint32 off		  = mFaceOffset[f];
