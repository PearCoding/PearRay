...
#include "geometry/TriangleIntersection_WT.h"
#include "math/Tangent.h"
#include "mesh/MeshBase.h"

namespace PR {
TriMeshWT::TriMeshWT(const std::string& name,
...
void TriMeshWT::checkCollisionLocal(const RayPackage& in, CollisionOutput& out)
{
	...
	mCollider
		->checkCollisionIncoherent(in, out,
								   [&](const RayPackage& in2, uint64 f, CollisionOutput& out2) {
									   ...
									   out2.Successful = TriangleIntersection::intersectWT(in2, p0, p1, p2, out2.HitDistance, uv);
...
void TriMeshWT::checkCollisionLocal(const Ray& in, SingleCollisionOutput& out)
{
	PR_ASSERT(mCollider && mBase, "Load before collision checks");
	mCollider
