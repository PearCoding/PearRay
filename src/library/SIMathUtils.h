...
template <class Unit>
inline typename std::enable_if<traits::is_unit<Unit>::value && !traits::is_scalar<Unit>::value, std::ostream &>::type
operator<<(std::ostream &os, const Unit &obj)
{
    os << obj.Value << " " << unit_to_string<Unit>();
    return os;
}
